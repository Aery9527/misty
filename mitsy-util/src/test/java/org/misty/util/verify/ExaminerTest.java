...
class ExaminerTest {
    ...
    private final Map<String, Number> targets = Collections.unmodifiableMap(new LinkedHashMap<String, Number>() {{
    ...
    @SuppressWarnings("UnnecessaryBoxing")
    private final Map<String, Number> numberOf0 = Collections.unmodifiableMap(new LinkedHashMap<String, Number>() {{
    ...
    @SuppressWarnings("UnnecessaryBoxing")
    Map<String, Number> numberOf1 = Collections.unmodifiableMap(new LinkedHashMap<String, Number>() {{
        put("Short1", new Short((short) 1));
        put("Integer1", new Integer(1));
        put("Long1", new Long(1L));
        put("Float1", new Float(1f));
    ...
    @SuppressWarnings("UnnecessaryBoxing")
    Map<String, Number> numberOf2 = Collections.unmodifiableMap(new LinkedHashMap<String, Number>() {{
...
//                .hasMessage(ExaminerMessage.refuseNullAndEmpty(term)); // XXX 不知道為什麼這邊操作會錯@@?
    ...
    public void test_requireInRange_Number() {
        ...
        inRangeFloors.putAll(this.numberOf0);
        inRangeFloors.putAll(this.numberOf1);

        Map<String, Number> inRangeCeilings = new LinkedHashMap<>();
        inRangeCeilings.putAll(this.numberOf2);
        inRangeCeilings.putAll(this.numberOf1);
        ...
        // test in range
        this.targets.forEach((targetTerm, targetNumber) -> {
        ...
        // test out range
        this.targets.forEach((targetTerm, targetNumber) -> {
            this.numberOf1.forEach((floorTerm, floorNumber) -> {
                this.numberOf0.forEach((ceilingTerm, ceilingNumber) -> {
                    try {
            ...
            this.numberOf2.forEach((floorTerm, floorNumber) -> {
                this.numberOf1.forEach((ceilingTerm, ceilingNumber) -> {
                    try {
                        Assertions.assertThatThrownBy(() -> Examiner.requireInRange(term, targetNumber, floorNumber, ceilingNumber))
                                .isInstanceOf(MistyException.class).is(condition)
                                .hasMessageContaining(ExaminerMessage.requireInRange(term, targetNumber, floorNumber, ceilingNumber));
